...
type Search struct {
	...
	// Testbed instrumentation (see simnet harness); no effect on protocol.
...
// SetCycle records which rollover this Search instance is (reach instrumentation).
...
// concurrent searches). They distinguish nodes/ads that entered a search table
// via the DHT routing table (seed, src==nil) versus referrals from other
// registrars' TOPICQUERY responses (src!=nil). Testbed instrumentation only.
var (
...
// SearchBucketStats returns mean occupancy per search-table bucket (0 = farthest
// from topic, last = closest), sampled when a search saturates, plus AddNodes
// rejection counts.
...
// Per-searcher reach sampling (testbed bottleneck analysis).
type regStat struct {
	firstCycle int
	nQueries   int
...
var (
	reachData    sync.Map // self enode.ID -> *reachSet
	reachEnabled bool
)
...
func reachSampled(id enode.ID) bool { return id[0] < 3 } // ~1% of nodes
...
func (s *Search) IsDone() bool {
	...
	// done. There is no more nodes to query. Sample per-bucket occupancy.
	for i := range s.buckets {
