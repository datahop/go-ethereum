...
// TestRegistrationHandleTicketResponseClampMin verifies that a registrar's
// tiny waitTime is clamped up to MinWaitTime. Without this, a misbehaving
// registrar could force a tight resend loop by quoting near-zero waits.
func TestRegistrationHandleTicketResponseClampMin(t *testing.T) {
	simclock := new(mclock.Simulated)
	cfg := testConfig(t)
	cfg.Clock = simclock
	...
	// Registrar quotes 100ms. Expect the registrant to clamp to MinWaitTime.
	start := simclock.Now()
	r.HandleTicketResponse(att, []byte("t"), 100*time.Millisecond)
	if want := start.Add(cfg.MinWaitTime); att.NextTime != want {
		t.Fatalf("NextTime not clamped to MinWaitTime: got %v, want %v (clamp from 100ms)",
			att.NextTime, want)
	}
	if att.totalWaitTime != cfg.MinWaitTime {
		t.Fatalf("totalWaitTime not clamped: got %v, want %v",
			att.totalWaitTime, cfg.MinWaitTime)
	}
}

// TestRegistrationHandleTicketResponseClampMax verifies that a registrar's
// excessive waitTime is clamped down to AdLifetime. Matches the paper's
// Algorithm 1 SLEEP(min(E, response.ticket.wait_for)).
func TestRegistrationHandleTicketResponseClampMax(t *testing.T) {
	simclock := new(mclock.Simulated)
	cfg := testConfig(t)
	cfg.Clock = simclock
	cfg.MinWaitTime = 30 * time.Second
	...
	// Registrar quotes 49 days. Expect clamp to AdLifetime.
	start := simclock.Now()
	r.HandleTicketResponse(att, []byte("t"), 49*24*time.Hour)
	if want := start.Add(cfg.AdLifetime); att.NextTime != want {
		t.Fatalf("NextTime not clamped to AdLifetime: got %v, want %v (clamp from 49 days)",
			att.NextTime, want)
	}
	if att.totalWaitTime != cfg.AdLifetime {
		t.Fatalf("totalWaitTime not clamped: got %v, want %v",
			att.totalWaitTime, cfg.AdLifetime)
	}
}
